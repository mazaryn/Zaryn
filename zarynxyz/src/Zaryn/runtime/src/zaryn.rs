	...
	pub fn validate_transaction(transaction: &Transaction) -> Result<ValidTransaction, &'static str> {
		...
		for input in transaction.inputs.iter() {
			if let Some(input_utxo) = <ZarynStore>::get(&input.outpoint) {
					...
					&simple_transaction,
					&Public::from_h256(input_zaryn.pubkey)
				), "signature must be valid" );
				total_input = total_input.checked_add(input_utxo.value).ok_or("input value overflow")?;
			} else {
				missing_zaryn.push(input.outpoint.clone().as_fixed_bytes().to_vec());
			}
		}
...
mod tests {
	...
	const GENESIS_ZARYN: [u8; 32] = hex!("79eabcbd5ef6e958c6a7851b36da07691c19bda1835a08f875aa286911800999");
	...
	fn test_simple_transaction() {
		new_test_ext().execute_with(|| {
			...
			let new_zaryn_hash = BlakeTwo256::hash_of(&(&transaction.encode(), 0 as u64));
	...
	fn attack_with_empty_transactions() {
		new_test_ext().execute_with(|| {
			assert_noop!(
				Zaryn::spend(Origin::signed(0), Transaction::default()), // an empty trx
				"no inputs"
			);
	...
	fn attack_by_double_counting_input() {
		new_test_ext().execute_with(|| {
			...
			assert_noop!(
				Zaryn::spend(Origin::signed(0), transaction),
